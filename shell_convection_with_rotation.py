...
To run and plot using e.g. 4 processes:
    $ mpiexec -n 4 python3 spherical_shell_test_2.py
...
dtype = np.float64
mesh = None
Omega = 0  # Angular velocity of the Sun (rad/s)
...
ez = dist.VectorField(coords, bases=shell)
ez['g'][1] = -np.sin(theta)
ez['g'][2] = np.cos(theta)
...
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) - b*er + lift(tau_u2) + 2*Omega*curl(u) = - u@grad(u)")
